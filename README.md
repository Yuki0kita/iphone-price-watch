...
買取1丁目の全カテゴリを走査し、買取価格が高い商品を洗い出す。

```bash
python3 -m src.scan --min-gap 20000
```

**一覧APIが返す定価は「発売時」の値**で、現在の定価ではない。商品登録時点のまま固定される。

```text
Steam Deck 有機EL 1TB   API 99,800 → 現在 167,980（2026年に2回値上げ）
PlayStation5 Pro       API 119,980 → 現在 137,980
```

そのため出力は**候補の洗い出しにしか使えない**。差額が大きく見えても、多くは値上げの反映漏れ。
実際に利益が出るかは、現在の実売価格（Yahoo!ショッピング）と突き合わせて判断する。
...
- 買取だけでなく仕入価格も履歴に残す（`retail_price`）。「値上がり前に買って値上がり後に売る」
  判断には両方の推移が要るため。グラフは買取を実線、仕入を破線で重ねて描く
- 買取価格の履歴を提供しているAPIは無い（買取1丁目にも買取Xにも無い）。
  過去に遡ることはできないので、監視を始めた日から貯まっていく
